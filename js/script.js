...
const container = document.getElementById("container")
  ...
  rec.innerHTML = `____________________________________________<br/>`
  rec.innerHTML += slot[index].recommendation
  element.addEventListener("click", ()=> message.innerHTML = slot[index].question);
...
function goToNext(){
  ...
  if(index >= (slot.length - 1)){
    index= 0
    img.style.transform = `rotate(-${slot[index].angle}deg)`;
...
function goToPrevious(){
  ...
  message.innerHTML=''
    if(index <= 0){
      let last = slot.length - 1
      //let restart = slot[index].angle + 24
      img.style.transform = `rotate(-${slot[last].angle}deg)`;
      img.style.transition = "all 1s";
      index= 14
      rec.innerHTML = `____________________________________________<br/>`
      rec.innerHTML += slot[index].recommendation
